...
import { ArrowUpDown, Bitcoin, RefreshCw, TrendingDown, TrendingUp } from "lucide-react"
...
// Mapeo de nombres de exchanges para mostrar nombres más amigables
...
export default function CryptoRates({ compact = false }: { compact?: boolean }) {
  ...
  const fetchRates = async () => {
    try {
      ...
      setError("No se pudieron cargar las cotizaciones. Intente nuevamente más tarde.")
      console.error(err)
    } finally {
      setLoading(false)
    }
  ...
  // Para comprar USDT: compra más baja
  const bestBuyRates = [...rates].sort((a, b) => a.compra - b.compra).slice(0, 3)
  // Para vender USDT: venta más alta
  const bestSellRates = [...rates].sort((a, b) => b.venta - a.venta).slice(0, 3)
    ...
    return (
      <div className="space-y-2">
        <div className="flex justify-between items-center">
          <h3 className="text-xs font-medium text-slate-300">Mejores opciones para USDT/ARS</h3>
          <Button
            onClick={fetchRates}
            variant="outline"
            size="sm"
          ...
          <Card className="bg-slate-800/30 border-slate-700/30">
            <CardContent className="p-2">
              <h4 className="text-xs font-medium mb-1 flex items-center gap-1">
                ...
                <span className="text-slate-300">Mejores para comprar USDT</span>
              </h4>
          ...
          <Card className="bg-slate-800/30 border-slate-700/30">
            <CardContent className="p-2">
              <h4 className="text-xs font-medium mb-1 flex items-center gap-1">
                ...
                <span className="text-slate-300">Mejores para vender USDT</span>
              </h4>
              <div className="space-y-1">
    ...
    <div>
        ...
        <Table className="text-xs">
          <TableHeader>
            <TableRow className="hover:bg-slate-700/30 border-slate-700/50">
              <TableHead className="text-slate-400 py-2 h-8">Exchange</TableHead>
          ...
          <TableBody>
            {sortedRates.map((rate, index) => (
              <TableRow key={index} className="hover:bg-slate-700/30 border-slate-700/50">
                <TableCell className="font-medium py-1.5 h-7">
                  {EXCHANGE_NAMES[rate.exchange] || rate.exchange}
                </TableCell>
      ...
      <div className="mt-2 text-[10px] text-slate-400 bg-slate-800/30 p-2 rounded-md border border-slate-700/30">
        <p className="flex items-center gap-1">
          ...
          Para mejores precios de compra de USDT, ordena la columna "Compra USDT" de menor a mayor
        </p>
        <p className="flex items-center gap-1 mt-0.5">
          <TrendingDown className="h-2.5 w-2.5 text-red-400" />
          Para mejores precios de venta de USDT, ordena la columna "Venta USDT" de mayor a menor
        </p>
      </div>
    </div>
  )
